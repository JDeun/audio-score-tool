...
def test_known_presets_resolve():
    ...
    assert presets.resolve_preset("quality").muscriptor_model == "large"
    assert presets.resolve_preset("quality").whisperx_model == "large-v3"


def test_auto_defaults_to_quality():
    assert presets.recommended_preset() == "quality"
    assert presets.resolve_preset("auto").key == "quality"
