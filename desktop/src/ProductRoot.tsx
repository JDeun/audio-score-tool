...
const sectionMeta: Record<ProductSection, { label: string; eyebrow: string; description: string }> = {
  new: {
    label: "새 악보",
    eyebrow: "NEW SCORE",
    description: "음원 파일이나 YouTube 링크를 넣으면 파트·코드·가사까지 자동으로 채보합니다.",
  },
  songs: {
    label: "곡 라이브러리",
    ...
    <div className="product-frame">
      <aside className="product-sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <span className="product-version">v0.6 · 로컬 우선</span>
        </div>
      </aside>
          ...
          <section className="onboarding-card" role="dialog" aria-modal="true" aria-labelledby="onboarding-title">
            ...
            {onboardingStep === 0 && (
              <div className="onboarding-content">
                ...
                <h2 id="onboarding-title">음원 한 곡을 출판 가능한 악보로</h2>
                <p>AudioScoreTool은 음원이나 YouTube에서 파트·코드·가사를 자동으로 채보하고, 앱 안에서 수정·조판해 PDF/MusicXML/MIDI로 내보내는 로컬 데스크탑 앱입니다.</p>
                <div className="onboarding-features">
                  <span>다중 악기 자동 채보</span><span>코드·가사 자동 생성</span><span>출판용 악보 편집</span>
                </div>
              </div>
            )}
            ...
            {onboardingStep === 1 && (
              <div className="onboarding-content">
                ...
                <p>모델은 로컬에서 실행됩니다. 아래 항목이 준비되면 바로 채보를 시작할 수 있습니다.</p>
                <div className="check-list">
                  ...
                  <div><span className={health?.system.hf_authenticated ? "check-good" : "check-warn"}>{health?.system.hf_authenticated ? "✓" : "!"}</span><strong>Hugging Face</strong><small>{health?.system.hf_authenticated ? "인증됨" : "MuScriptor 모델 인증 필요"}</small></div>
                </div>
              </div>
            )}
            ...
            {onboardingStep === 2 && (
              <div className="onboarding-content">
                ...
                <p>로컬 음원을 끌어다 놓거나 YouTube 링크를 붙여 넣어 시작하세요. 결과는 자동으로 곡 라이브러리에 저장됩니다.</p>
                <div className="onboarding-actions-grid">
                  <button onClick={() => finishOnboarding("new")}><strong>새 악보 만들기</strong><span>음원 또는 YouTube에서 시작</span></button>
